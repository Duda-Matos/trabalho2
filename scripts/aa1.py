# -*- coding: utf-8 -*-
"""
Created on Sat Jun  7 14:35:26 2025
...
import numpy as np
import sys
import locale # Importar locale para formatação de data
import matplotlib.dates as mdates # Importar mdates para formatação de data

# %% 1: Imports e Verificação de Ambiente
print(f"Python executable being used: {sys.executable}")
print(f"Python paths: {sys.path}")
try:
    import dask
    print(f"Dask successfully imported from within script. Version: {dask.__version__}")
except ImportError as e:
    print(f"Dask import failed within script: {e}")
    sys.exit("Erro: Dask não disponível. Certifique-se de que está instalado e o ambiente Python correto está ativo.") # Adicionado sys.exit
...
# Nome da variável a ser analisada (verifique quais estão disponíveis)
VARIAVEL_INTERESSE = "DTDTTOT"  # pode trocar para "T2M" se preferir temperatura 2m
...
ds = None # Inicializa ds como None
try:
    # Abre múltiplos arquivos de forma eficiente e paralela
    # Mantendo engine='netcdf4' e parallel=True, conforme as discussões anteriores
    ds = xr.open_mfdataset(padrao_arquivos, combine='by_coords', parallel=True, engine='netcdf4')
    print("Arquivos carregados com sucesso.")
    print("Variáveis disponíveis:", list(ds.data_vars))
except Exception as e:
    print(f"Erro ao carregar os arquivos NetCDF: {e}")
    print("Verifique se os arquivos estão corrompidos ou se o ambiente Conda está configurado corretamente com netCDF4.")
    sys.exit("Falha no carregamento dos dados. Encerrando script.") # Termina o script aqui

# --- VERIFICAÇÃO CRÍTICA: SE DS NÃO FOI CARREGADO, O SCRIPT DEVE PARAR ---
if ds is None:
    sys.exit("Dataset 'ds' não foi carregado com sucesso. Verifique os erros anteriores.")
...
shape = None # Inicializa shape como None
try:
    ...
    sys.exit("Falha no carregamento/processamento do shapefile. Encerrando script.") # Termina o script aqui

# --- VERIFICAÇÃO CRÍTICA: SE SHAPE NÃO FOI CARREGADO, O SCRIPT DEVE PARAR ---
if shape is None:
    sys.exit("Shapefile não carregado com sucesso. Verifique os erros anteriores.")
...
data_recortado = None # Inicializa data_recortado como None
try:
    ...
    sys.exit("Falha no recorte geográfico. Encerrando script.") # Termina o script aqui

# --- VERIFICAÇÃO CRÍTICA: SE DATA_RECORTADO NÃO FOI RECORTADO, O SCRIPT DEVE PARAR ---
if data_recortado is None:
    sys.exit("Dados não recortados com sucesso. Verifique os erros anteriores.")
...
plt.title(f"Média da Tendência de Temperatura em {NIVEL_PRESSAO} hPa para o Brasil em 2024", fontsize=16, pad=20)
...
for var_nome in VARIAVEIS_CONTRIBUICAO:
    if var_nome in ds:
        ...
        try:
            ...
            print(f"Aviso: Erro ao processar a variável de contribuição '{var_nome}': {e}")
    else:
        print(f"Aviso: Variável de contribuição '{var_nome}' não encontrada no dataset.")
